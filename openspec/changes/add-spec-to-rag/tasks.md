...
- [ ] 4.4 手動驗證：對一份 `openspec/specs/**/spec.md` 做一個小幅修改並 commit，確認 hook 觸發、`GET /v1/documents`（或 AnythingLLM 網頁介面）可看到新版本文件已寫入 `sdd-current-specs`，且原本存在的舊版本已被移除（若該檔案先前已匯入過）。
- [ ] 4.5 手動驗證：暫停本機 AnythingLLM container（`docker stop anythingllm`），對相關路徑做修改並 commit，確認 commit 本身仍成功完成、終端機沒有因此顯示令人誤解的失敗訊息；驗證完成後重新啟動 container（`docker start anythingllm`）。
