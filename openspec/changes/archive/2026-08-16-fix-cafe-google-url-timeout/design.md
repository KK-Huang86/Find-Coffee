...
**Goals:**
- 讓多筆結果的照片載入不再逐間同步等待外部解析，**同步等待外部圖片來源解析**的時間不隨未快取店家數量疊加（消除原本每間最多 2 秒的 Google HTTP 同步等待疊加問題，即本次真正的動機）
...
- **不消除 `_trigger_s3_upload` 本身在多筆結果迴圈中逐間同步執行的 DB 查詢與 Celery `.delay()` 呼叫**：這兩個動作仍是 O(N)（N = 未快取店家數），只是成本量級遠低於原本的 Google HTTP 同步解析（毫秒級 vs. 最多 2 秒級），且 broker 異常時 `.delay()` 理論上仍可能拖慢回應。要做到嚴格零額外成本，需要把觸發判斷改成批次處理或移出 webhook 同步路徑，屬於更大的架構調整，本次不處理（見 Risks/Trade-offs）。
...
- [風險] 多筆結果迴圈中，每個未快取店家仍會同步執行 `_trigger_s3_upload` 的 DB 查詢與 Celery `.delay()`，此開銷隨未快取店家數量線性增加，並非嚴格零成本；若 Celery broker 異常（連線逾時、重試），`.delay()` 可能拖慢整體回應 → 緩解：此開銷的量級（單次 DB 查詢 + 訊息發布，通常毫秒級）遠低於原本要解決的問題（Google HTTP 同步解析，單次最多 2 秒），已消除本次變更真正要處理的疊加風險；spec 措辭已對應收斂為「不因未快取店家數量增加而累加**同步等待外部圖片來源解析**的時間」，不承諾背景快取觸發本身零成本。若未來 broker 穩定性成為實際問題，可另開 change 評估批次處理或移出同步路徑（屬於更大的架構調整，本次不做，見 Goals/Non-Goals）。
