...
- [風險] 移除觸發條件的 gate 後，`_trigger_s3_upload` 被呼叫的頻率會比現行更高（包含原本因逾時而完全不會觸發的情況）→ 緩解：`_trigger_s3_upload` 內部既有的 `not cafe.photo_s3_url` 檢查、以及 Celery task 內部再次檢查 `if cafe.photo_s3_url: skip`，兩層檢查確保多出來的觸發只是「多排一次幾乎立即被跳過的 task」，不會造成重複下載或資料不一致，符合專案冪等性規範。
- [風險] 高併發下（多個使用者同時搜尋同一間未快取店家）仍可能有多個請求同時通過 `not cafe.photo_s3_url` 檢查、各自 `.delay()` 一次 → 緩解：這與現行行為一致（既有程式碼本來就有此競態），非本次變更引入的新風險，且後果僅止於重複下載同一張圖片一次、無資料損毀風險，本次不額外處理（超出本次變更範圍）。
