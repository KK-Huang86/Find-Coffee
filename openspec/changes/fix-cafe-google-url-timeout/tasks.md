...
- [ ] 1.1 確認目前在 `feature/fix-cafe-google-url-timeout` 分支，且 `git status` 乾淨。
- [ ] 1.2 執行 `uv run pytest line_bot/tests/builders/test_shop_flex_message.py -q`，記錄基準測試數與全數通過（基準綠燈）。
- [ ] 1.3 執行 `uv run pytest -q` 全套測試，記錄基準總測試數與全數通過。
...
- [ ] 2.6 於 `TestCreateShopFlexMessage` 新增測試：`create_shop_flex_message(info, is_multiple=True)` 呼叫 `get_photo_url` 時帶入 `allow_sync_resolve=False`；`is_multiple=False`（或不傳，預設值）時帶入 `allow_sync_resolve=True`（用 `patch.object(FlexMessageBuilder, 'get_photo_url')` 驗證呼叫參數）。
- [ ] 2.7 執行 `uv run pytest line_bot/tests/builders/test_shop_flex_message.py -q`，確認 2.2-2.6 新增/調整的測試在實作變更前為紅燈，且失敗原因確實是「尚未支援 `allow_sync_resolve` 參數 / 觸發條件尚未解耦」等預期原因（例如 `TypeError: unexpected keyword argument`），而非測試本身寫錯；2.1 修改後的測試此時應仍是紅燈（因實作尚未變更）。
...
- [ ] 4.3 執行 `uv run pytest -q` 全套測試，確認全數通過，並比對總測試數 = 第 1.3 步基準數 + 本次新增測試數（2.2-2.6，扣除 2.1 為既有測試修改非新增）。
